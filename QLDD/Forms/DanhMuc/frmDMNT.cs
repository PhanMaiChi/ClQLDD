...
{
    public partial class frmDMNT : DevComponents.DotNetBar.Office2007Form
    {
        int SoBG = 0;
            ...
            try
            {
                for (int i = 0; i < grvKQ.RowCount -1; i++)
                {
                    if (Connect.GetStr(grvKQ.GetRowCellValue(i,colTT)) == "1")
                    {
                        ...
                        cmd.Parameters.Add("@Theodoi", OleDbType.VarChar).Value = grvKQ.GetRowCellValue(i, colTheodoi);
                        Connect.LuuDL(cmd);
                    }
